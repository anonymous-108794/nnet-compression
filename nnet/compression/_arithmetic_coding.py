# Modifications Copyright (c) Anonymous Authors
...
#
""" This module provides support for arithmetic coding. """

import numpy as np
import struct
from ._bit_manipulation import BitInputStream, BitOutputStream
...
class SimpleFrequencyTable(FrequencyTable):
    ...
    def __init__(self, freqs):
        if isinstance(freqs, FrequencyTable):
            ...
            self.frequencies = np.array([freqs.get(i) for i in range(numsym)], dtype=np.int32)
        else:  # Assume it is a sequence type
            self.frequencies = np.array(freqs, dtype=np.int32)  # Make copy
    ...
    def _init_cumulative(self):
        self.cumulative = np.zeros(len(self.frequencies) + 1, dtype=np.int32)
...
class AdaptiveArithmeticCompression:
    @staticmethod
    def _make_initial_frequencies(num_symbols):
        initial_frequencies = FlatFrequencyTable(num_symbols + 1)
        frequencies = SimpleFrequencyTable(initial_frequencies)
        return frequencies

    def compress(self, stream, values):
        num_symbols = np.max(values) + 1

        if num_symbols > 2 ** 16:
            raise ValueError('Too many symbols!')

        stream.write(struct.pack('!H', num_symbols))
        bitout = BitOutputStream(stream)

        frequencies = AdaptiveArithmeticCompression._make_initial_frequencies(num_symbols + 1)
        encoder = ArithmeticEncoder(bitout)

        for v in values:
            encoder.write(frequencies, v)
            frequencies.increment(v)

        encoder.write(frequencies, num_symbols + 1)  # Write EOF
        encoder.finish()

        bitout.close()

    def decompress(self, stream):
        num_symbols = struct.unpack('!H', stream.read(2))[0]

        frequencies = AdaptiveArithmeticCompression._make_initial_frequencies(num_symbols + 1)
        bitin = BitInputStream(stream)

        decoder = ArithmeticDecoder(bitin)

        data = []

        while True:
            symbol = decoder.read(frequencies)
